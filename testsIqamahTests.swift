...
final class PrayerCalculatorTests: XCTestCase {
    ...
    func testDateBoundaryRecalculation() throws {
        ...
        let date1 = try XCTUnwrap(Calendar.current.date(from: date1Components))

        let date2Components = DateComponents(year: 2024, month: 7, day: 1, hour: 12)
        let date2 = try XCTUnwrap(Calendar.current.date(from: date2Components))
...
final class SettingsManagerTests: XCTestCase {
    ...
    func testCalculationMethodPersistence() {
        let settings = SettingsManager.shared
...
final class CitiesDatabaseTests: XCTestCase {
    ...
    func testFindCountry() {
        guard case let .success(database) = CitiesLoader.shared.load() else {
    ...
    func testClosestCity() {
        ...
        if let city = closestCity {
            ...
            XCTAssertLessThan(distance, 500_000, "Closest city should be within 500km (got \(distance / 1000)km)")
        }
    }
    ...
    func testCityDistance() throws {
        ...
        XCTAssertGreaterThan(distance, 5_000_000, "Distance should be > 5000km")
        XCTAssertLessThan(distance, 6_000_000, "Distance should be < 6000km")
    }
}
