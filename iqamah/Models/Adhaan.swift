...
struct Adhaan: Identifiable, Codable, Hashable {
    ...
    static var alertTones: [Adhaan] {
        ...
        return known.compactMap { entry in
            for ext in entry.exts {
                if Bundle.main.url(forResource: entry.id, withExtension: ext) != nil {
                    return Adhaan(id: entry.id,
    ...
    static var adhaanRecordings: [Adhaan] {
        (1...10).compactMap { i in
            let id = "adhaan_\(i)"
            for ext in ["mp3", "m4a", "aac", "aiff"] {
                if Bundle.main.url(forResource: id, withExtension: ext) != nil {
                    return Adhaan(id: id,
                                  displayName: "Adhaan \(i)",
                                  filename: "\(id).\(ext)")
                }
            }
            return nil
